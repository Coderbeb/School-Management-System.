...
interface LeaveRequest {
    ...
    role: string; leave_type: string; from_date: string; to_date: string;
    reason: string; status: 'pending' | 'approved' | 'rejected';
    reviewed_by_name?: string; reviewed_at?: string; review_remarks?: string;
    created_at: string;
}
...
export default function StudentLeaveManagementPage() {
    ...
    // Action modal state
    const [actionModal, setActionModal] = useState<{ requestId: string; action: 'approved' | 'rejected'; studentName: string } | null>(null);
    ...
    const fetchLeaveRequests = async (token?: string) => {
        ...
        try {
            const res = await fetch('/api/leave-requests?roleFilter=student', {
    ...
    const getLeaveTypeBadge = (type: string) => {
        const map: Record<string, { bg: string; label: string }> = {
            sick: { bg: 'bg-red-100 text-red-800', label: 'Sick Leave' },
            casual: { bg: 'bg-blue-100 text-blue-800', label: 'Casual Leave' },
            personal: { bg: 'bg-purple-100 text-purple-800', label: 'Personal' },
            other: { bg: 'bg-gray-100 text-gray-700', label: 'Other' },
        };
        const entry = map[type?.toLowerCase()] || map['other'];
        return <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${entry.bg}`}>{entry.label}</span>;
    };

    const pendingCount = requests.filter(r => r.status === 'pending').length;
    const approvedCount = requests.filter(r => r.status === 'approved').length;
    const rejectedCount = requests.filter(r => r.status === 'rejected').length;
    const totalDaysPending = requests.filter(r => r.status === 'pending').reduce((s, r) => s + calculateDays(r.from_date, r.to_date), 0);
    ...
    const tabs = [
        { key: 'pending' as const, label: 'Pending', count: pendingCount, icon: Clock, color: 'amber' },
        { key: 'approved' as const, label: 'Approved', count: approvedCount, icon: CheckCircle, color: 'emerald' },
        { key: 'rejected' as const, label: 'Rejected', count: rejectedCount, icon: XCircle, color: 'red' },
    ];
            ...
            <main className="flex-1 max-w-4xl mx-auto w-full px-4 py-8 mt-16">
                ...
                <div className="relative overflow-hidden rounded-3xl bg-gray-900 text-white p-6 sm:p-8 mb-6 shadow-xl">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="flex items-center gap-2 mb-2">
                            <span className="text-indigo-400 font-semibold tracking-wide uppercase text-sm">Student Management</span>
                        </div>
                        <h1 className="text-2xl sm:text-3xl font-bold mb-2 flex items-center gap-3">
                            Student Leave Requests <GraduationCap className="w-7 h-7 text-indigo-400" />
                        </h1>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                    <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm text-center">
                        ...
                        <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-1">Total Requests</div>
                    </div>
                    ...
                    <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-4 shadow-sm text-center">
                        ...
                        <div className="text-[10px] font-bold text-emerald-500 uppercase tracking-wider mt-1">Approved</div>
                    </div>
                    <div className="bg-indigo-50/50 border border-indigo-100 rounded-2xl p-4 shadow-sm text-center">
                        <div className="text-2xl font-black text-indigo-700">{totalDaysPending}</div>
                        <div className="text-[10px] font-bold text-indigo-500 uppercase tracking-wider mt-1">Pending Days</div>
                    </div>
                </div>

                {/* Toast */}
                {toast && (
                    <div className={`mb-6 flex items-center gap-2 p-3.5 rounded-xl text-xs font-medium shadow-sm ${
                ...
                {/* Tabs */}
                <div className="flex items-center gap-2 mb-4 bg-white rounded-2xl p-1.5 border border-gray-150 shadow-sm">
                    {tabs.map((tab) => (
                ...
                <div className="relative mb-4">
                    ...
                    <input type="text" placeholder="Search by student name..."
                        ...
                        className="w-full h-10 pl-10 pr-4 rounded-xl border border-gray-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all" />
                </div>

                {loading ? (
                    <div className="flex flex-col items-center justify-center py-16 gap-4">
                        <Loader2 className="w-8 h-8 animate-spin text-indigo-600" />
                        <p className="text-gray-500 font-medium">Loading student leave requests...</p>
                    </div>
                    ...
                    <div className="text-center py-16 bg-white rounded-3xl border border-dashed border-gray-200 shadow-sm">
                        ...
                        <p className="text-gray-500 font-medium">No {activeTab} student leave requests.</p>
                        <p className="text-gray-400 text-xs mt-1">
                            {searchQuery ? 'Try a different search term.' : 'All clear! Check other tabs for more requests.'}
                        </p>
                    </div>
                ) : (
                                ...
                                <div className="p-5">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-indigo-100 text-indigo-700">Student</span>
                                                </div>
                                    ...
                                    {request.status === 'pending' && (
                                        <div className="flex items-center gap-2 pt-2 border-t border-gray-100">
                                            <button
                                                onClick={() => { setActionModal({ requestId: request.id, action: 'approved', studentName: `${request.first_name} ${request.last_name}` }); setActionRemarks(''); }}
                                            ...
                                            <button
                                                onClick={() => { setActionModal({ requestId: request.id, action: 'rejected', studentName: `${request.first_name} ${request.last_name}` }); setActionRemarks(''); }}
                ...
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 animate-fade-in">
                    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-sm overflow-hidden">
                        <div className={`px-5 py-4 flex items-center justify-between text-white ${
                            ...
                            <button onClick={() => setActionModal(null)} className="text-white/80 hover:text-white transition-colors">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        <div className="p-5 space-y-4">
                            <p className="text-sm text-gray-700">
                                {actionModal.action === 'approved' ? 'Approve' : 'Reject'} leave request from{' '}
                                <strong>{actionModal.studentName}</strong>?
                            </p>
                            <div>
                                ...
                                <textarea
                                    ...
                                    rows={3}
                                    className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-xs bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none"
                                />
                            </div>
                            <div className="flex items-center gap-2">
